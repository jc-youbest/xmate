...
| C-029 | ToolPickerHost | App-wide PKToolPicker singleton; registers/unregisters XmateCanvasView instances and binds the picker to whichever canvas DrawingSessionManager designates active via `setActiveCanvas`. Thin: it no longer chooses first responder or re-anchors on its own — all activation decisions come from C-030. Also a PKToolPickerObserver that explicitly pushes every selected-tool change into all registered canvases and re-stamps the current tool on register / setActiveCanvas — tool state is convergent, not dependent on PencilKit's implicit first-responder delivery, which missed color changes during Single Page flip/zoom responder churn (F-051 / F-053 / F-056) |
